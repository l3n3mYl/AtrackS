...
class SwipeButton extends StatefulWidget {
  ...
  final Color sliderBaseColor;
  final Color sliderButtonColor;
  final Color borderColor;

  const SwipeButton({
    Key key,
    this.sliderBaseColor,
...
class SwipeButtonState extends State<SwipeButton>
      ...
      height: widget.height,
        child: Stack(
          ...
          children: <Widget>[
            DecoratedBox(
              decoration: BoxDecoration(
                color: widget.sliderBaseColor,
                borderRadius: widget.borderRadius,
              ),
              child: ClipRRect(
                clipper: _SwipeButtonClipper(
                    animation: _controller, borderRadius: widget.borderRadius),
                borderRadius: widget.borderRadius,
            ...
            AnimatedBuilder(
              ...
              builder: (BuildContext context, Widget child) {
                return Align(
                  alignment: Alignment((_controller.value * 2) - 1.0, 0.0),
                  child: child,
                );
              },
              child: GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: widget.sliderButtonColor,
                    borderRadius: widget.borderRadius,
                  ),
                  child: widget.thumb,
                ),
  ...
  void _onDragEnd(DragEndDetails details) {
    ...
    final simulation =
        _SwipeSimulation(acceleration, _controller.value, 1.0, velocity);
...
class _SwipeButtonClipper extends CustomClipper<RRect> {
  ...
  RRect getClip(Size size) {
    return borderRadius.toRRect(Rect.fromLTRB(
        size.width * animation.value, 0.0, size.width, size.height));
  }
